...
<template>
  ...
  <CContainer class="px-4" lg>
    ...
    <div v-else>
      ...
      <div v-if="!companyPackage" class="card border-warning shadow-sm mb-3">
        <div class="card-body bg-warning-subtle py-2 px-3">
          <div class="d-flex align-items-center mb-1">
            <div class="icon-bg bg-warning rounded-circle p-1 me-2">
              <fa-icon
            ...
            <h6 class="mb-0 text-warning-emphasis fw-semibold">
              {{ $t('subscription.manager.empty.title') }}
            </h6>
          </div>
          <p class="mb-0 text-warning-emphasis small">
            {{ $t('subscription.manager.empty.text') }}
          </p>
        </div>
      </div>
      ...
      <div v-if="packageList.length > 0">
        <div class="d-flex justify-content-between align-items-center mb-3">
          ...
          <div class="btn-group">
            <button
              class="btn btn-sm"
              :class="{
                'btn-success text-white': period === 'monthly',
                'btn-outline-success': period !== 'monthly'
              }"
            ...
            <button
              class="btn btn-sm"
              :class="{
                'btn-success text-white': period === 'yearly',
                'btn-outline-success': period !== 'yearly'
              }"
              @click="period = 'yearly'"
            >
        ...
        <div class="row g-3">
          <div class="col-lg-3 col-md-6 mb-3" v-for="pack in packageList" :key="pack.id">
              ...
              <div class="card-body p-3">
                ...
                <div class="mb-2">
                  <div v-if="period === 'monthly'">
                    <h5 class="mb-0" v-if="pack.price > 0">
                      {{ pack.price }}$
                      <span class="text-muted" style="font-size: 0.7rem">{{
                        $t('subscription.manager.planPicker.period.perMonth')
                      }}</span>
                    </h5>
                    <h5 class="mb-0 text-success" v-else>
                      {{ $t('subscription.manager.planPicker.price.free') }}
                    </h5>
                  </div>
                  <div v-else>
                    <h5 class="mb-0" v-if="pack.yearlyPrice > 0">
                      {{ pack.yearlyPrice }}$
                      <span class="text-muted" style="font-size: 0.7rem">{{
                        $t('subscription.manager.planPicker.period.perYear')
                      }}</span>
                    </h5>
                    <h5 class="mb-0 text-success" v-else>
                      {{ $t('subscription.manager.planPicker.price.free') }}
                ...
                <div class="mb-2">
                  <div class="d-flex justify-content-between mb-1" style="font-size: 0.8rem">
                    <div>
                      <fa-icon icon="building" fixed-width class="text-muted me-1" />
                      {{ $t('subscription.manager.planPicker.features.branches') }}
                    </div>
                    <div class="text-success" v-if="pack.branches === 0">
                      {{ $t('subscription.common.unlimited') }}
                    </div>
                    <div v-else>{{ pack.branches }}</div>
                  </div>

                  <div class="d-flex justify-content-between mb-1" style="font-size: 0.8rem">
                    <div>
                      <fa-icon icon="user" fixed-width class="text-muted me-1" />
                      {{ $t('subscription.manager.planPicker.features.experts') }}
                    </div>
                    <div class="text-success" v-if="pack.experts === 0">
                      {{ $t('subscription.common.unlimited') }}
                    </div>
                    <div v-else>{{ pack.experts }}</div>
                  </div>

                  <div class="d-flex justify-content-between mb-1" style="font-size: 0.8rem">
                    <div>
                      <fa-icon icon="list-check" fixed-width class="text-muted me-1" />
                      {{ $t('subscription.manager.planPicker.features.procedures') }}
                    </div>
                    <div class="text-success" v-if="pack.procedures === 0">
                      {{ $t('subscription.common.unlimited') }}
                    </div>
                    <div v-else>{{ pack.procedures }}</div>
                  </div>

                  <div class="d-flex justify-content-between mb-1" style="font-size: 0.8rem">
                    <div>
                      <fa-icon icon="file-text" fixed-width class="text-muted me-1" />
                  ...
                  <div class="d-flex justify-content-between mb-1" style="font-size: 0.8rem">
                    <div>
                      <fa-icon icon="ban" fixed-width class="text-muted me-1" />
              ...
              <div class="card-footer bg-white border-top-0 p-2">
                <div class="d-grid">
                  <button
                    ...
                    @click="subscribeToPackage(pack.id)"
                  >
                ...
                <div
                  class="text-success text-center mt-1"
                  v-if="pack.trialDays > 0 && !company.trialUsed"
                >

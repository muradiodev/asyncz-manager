<template>
  ...
  <div v-if="companyUser">
    <h3>
      Permissions for <span class="text-primary">{{ companyUser.name }}</span> <code>[{{companyUser.role}}]</code>
    </h3>
    ...
    <div v-if="!editing">
      ...
      <button class="btn-outline-custom" @click.prevent="editing = true">Edit Permissions</button>
  ...
  <div v-else>
    <p class="alert alert-info">Loading user...</p>
  </div>
</template>
